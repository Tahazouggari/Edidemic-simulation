...
int mq_timedsend(mqd_t mqd, const char *ptr, size_t len, unsigned prio,
    ...
    struct mymsg_hdr *nmsghdr;
    struct mymsg_hdr *pmsghdr;
    ...
    if (attr->mq_curmsgs == 0) {
        if (mqhdr->mqh_pid != 0 && mqhdr->mqh_nwait == 0) {
            ...
            if (sigev->sigev_notify == SIGEV_SIGNAL) {
                /* sigqueue does not exit on macOS, but it looks like it is
                 * enough if we just send a signal with kill to make simple
                 * tests pass. If a user does not use mq_notify this can be
                 * considered an unused branch.
    ...
    if ((freeindex = mqhdr->mqh_free) == 0) {
        log_info("mq_send: curmsgs = %ld; free = 0\n", attr->mq_curmsgs);
    }
